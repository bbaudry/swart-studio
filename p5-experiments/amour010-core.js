...
function fleur3(cx, cy, r1, r2) {
    ...
    while (a1 < a1init + 360) {
        ...
        r3 = r2 * noise(xoff)
        x2 = cx + r3 * cos(a1); y2 = cy + r3 * sin(a1)
        if (r3 < r1) {
        ...
        a2 = a1 + anglemax
        r3 = r2 * noise(xoff)
        x3 = cx + r1 * cos(a2); y3 = cy + r1 * sin(a2)
...
function exterieur(cx, cy, initialindex) {
    ...
    for (i = initialindex; i < initialindex + outerradii.length - 1; i++) {
        ...
        a1 = outerradii[index].angle; r1 = outerradii[index].rayon
        x1 = cx + r1 * cos(a1); y1 = cy + r1 * sin(a1)
        a2 = outerradii[index + 1].angle; r2 = outerradii[index + 1].rayon
        x2 = cx + r2 * cos(a2); y2 = cy + r2 * sin(a2)
        x1 < cx ? x3 = x1 - (extension * noise(xoff)) : x3 = x1 + extension * noise(xoff)
        xoff += xinc
        line(x1, y1, x3, y1); line(x3, y1, x3, y2); line(x3, y2, x2, y2)
}}
