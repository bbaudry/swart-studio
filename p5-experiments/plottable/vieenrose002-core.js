...
function ribbon() {
    ...
    for (i = 0; i < penwidth*52; i += penwidth) {
        ...
        cy1 = y1-i*1.3//*noise(xoff)//leftmargin + actualwidth * 0.1 * (1 - 2 * noise(xoff)); 
        xoff += xinc
        cx2 = topmargin + actualheight * 0.6 * noise(xoff); xoff += xinc
        cy2 = leftmargin + actualwidth *0.8 +i// * (1-2*noise(xoff));xoff+=xinc
        x2 = bottommargin
        y2 = leftmargin + actualwidth * 0.32 + i * 2.5// * noise(xoff); xoff += xinc
...
function ribbon2() {
    ...
    for (i = 0; i < penwidth*52; i += penwidth) {
        ...
        x2 = bottommargin
        y2 = leftmargin + actualwidth * 0.05 + i * 2// * noise(xoff); xoff += xinc
            line(x1,y1,cx1,cy1);line(cx1,cy1,cx2,cy2);line(cx2,cy2,x2,y2)
        beginShape()
        vertex(x1, y1)
        //bezierVertex(cx1, cy1, cx2, cy2, x2, y2)
