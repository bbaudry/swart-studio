...
function pick(){
	ryoji = letters[floor(random()*3)]
	ikeda = ryoji[floor(random()*ryoji.length)]
}
...
function sound(){
    ...
    osc1.start();
	radarLoop1.start(); radarLoop2.start();
...
//	rect(cx-424,cy-42,848,84)
	...
	for(i=0;i<h;i+=random()*42){
	//	rect(cx-rick/2,i,rick,random()*2)
	}
	rick++
	pick()
	vera=random(15)
	molnar=exp(random(5))
	gen=ikeda.x+molnar*cos(ikeda.alpha)
	art=ikeda.y+molnar*sin(ikeda.alpha)
