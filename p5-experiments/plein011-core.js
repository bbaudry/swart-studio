...
function hal() {
    let ax1, ay1, px1, py1, px2, py2, ax2, ay2, max, magicy, magicx
    max=actualheight*0.5
    ...
    for (i = 0; i < max; i++) {
        ...
        ax2 = rightmargin
        ay2 = bottommargin - i * magicy * noise(xoff); xoff+=xinc
        px1 = ax1 + actualwidth * magicx * i * noise(xoff); xoff+=xinc
        py1 = ay1 - (ay1-topmargin) * i * 0.005  * noise(xoff); xoff+=xinc
