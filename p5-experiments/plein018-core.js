...
function grille() {
    ...
    random()<0.5?splitquad(x4,y1,x1-gap,y1,x1-gap,y2-gap,x4,y4-gap):splittri(x1-gap,y1,x1-gap,y2-gap,x4,y4-gap)
...
function splitquad(x1,y1,x2,y2,x3,y3,x4,y4){
    let resx,resy,i,j,x,y,largeur,hauteur
    resx=Math.floor(random(2,6))
    resy=Math.floor(random(2,6))
    largeur=(x2-x1)/resx
    hauteur=(y3-y1)/resy
    for(i=0;i<resx;i++){
        x=lerp(x1,x2,i/resx)
