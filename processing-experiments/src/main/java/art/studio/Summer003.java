...
public class Summer003  extends PApplet {
    ...
    private void initCoords(){
        ...
        for(int i=0; i<7; i++){
            ...
            float cx = (float)(0.5*w);onepoint.add(cx);//(float)(0.2*w+alea.nextFloat()*0.6*w); onepoint.add(cx);
            float cy = (float)(0.5*h);onepoint.add(cy);//(float)(0.2*h+alea.nextFloat()*0.6*h); onepoint.add(cy);
    ...
    public static void main(String[] args) {
        String[] processingArgs = { "Summer003" };
        Summer003 mySketch = new Summer003();
